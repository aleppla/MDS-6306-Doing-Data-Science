...
```{r}
irisVersVirg = iris %>% filter(Species != "setosa")
...
df1 = data.frame(Sepal.Length = 6.10 , Sepal.Width = 2.50 )
knn(irisVersVirg[,c(1,2)], df1, irisVersVirg$Species, k = 5, prob = TRUE)
knn(irisVersVirg[,c(1,2)], df1, irisVersVirg$Species, k = 15, prob = TRUE)

df2 = data.frame(Sepal.Length = 6.20 , Sepal.Width = 4.9 )
knn(irisVersVirg[,c(1,2)], df2, irisVersVirg$Species, k = 5, prob = TRUE)
knn(irisVersVirg[,c(1,2)], df2, irisVersVirg$Species, k = 15, prob = TRUE)
```
...
classifications = knn.cv(df[,1:2], df$Qualify, k = 3) #CV=cross validation, leave one out
classifications
...
#Async 6.6 Exercise
classifications1 = knn.cv(iris[,1:2], iris$Species, k = 10) #CV=cross validation, leave one out
confusionMatrix(classifications1,iris$Species)

classifications2 = knn.cv(irisVersVirg[,1:2], irisVersVirg$Species, k = 10) #CV=cross validation, leave one out
confusionMatrix(classifications2,irisVersVirg$Species)
```
...
credit = read.csv('C:/Users/aleppla/GitRepos/MDS-6306-Doing-Data-Science/Unit 6/Credit Default.csv',header = TRUE)

#make response a factor rather than 0,1
...
MaxAcc = knn(train[,c(1,2)],test[,c(1,2)],train$Species, prob = TRUE, k = 34)
confusionMatrix(table(MaxAcc,test$Species))

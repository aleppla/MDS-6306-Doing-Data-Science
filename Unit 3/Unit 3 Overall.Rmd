...
install.packages("GGally")
library(GGally)
fifa = read.csv("C:/Users/aleppla/GitRepos/MDS-6306-Doing-Data-Science/Unit 3/FIFA Players.csv", header = TRUE)
head(fifa)
...
# With missing values in BC removed in mean call (na.rm="NA remove")
...
fifa %>% filter(!is.na(BallControl)) %>% group_by(Position) %>% summarize(meanBC = mean(BallControl), count = n()) %>% print(n = 28)
...
fifa_BC$Position = factor(fifa_BC$Position, level = fifa_BC$Position[order(fifa_BC$meanBC,decreasing=T)])
...
### Splits a continuous variable into factor segments/bins
```{r}
mpg$displ_Factor = cut(mpg$displ, breaks = c(1,4,6,8), labels = c("Low","Medium","High"))

mpg %>% ggplot(aes(x = hwy, y = cty, color = displ_Factor)) +geom_point() + ggtitle("City MPG v. Highway MPG by Displacement")
...
df
sapply(df,function(x) mean(x)) #applies a function to each data frame column (not just column x)

#Toy Example
# Bring in MissingValueToy.csv as ’a’
a = read.csv('C:/Users/aleppla/GitRepos/MDS-6306-Doing-Data-Science/Unit 3/MissingDataToy.csv',header = TRUE)
...
#Async 3.5 - Replace z=0 and z>20 with NA in diamonds data set
diamonds1 = diamonds %>%  replace_with_na(replace= list(z=0))
diamonds1 = diamonds1 %>%  replace_with_na_at(.vars = 'z', condition= ~.x>20)
gg_miss_var(diamonds1)
#In 1 line of code
diamonds2 = diamonds %>%  replace_with_na_at(.vars = 'z', condition= ~(.x==0 | .x>20))
gg_miss_var(diamonds2)
...
p = diamonds %>% ggplot(aes(x = z)) + geom_histogram()
ggplotly(p)
...
diamonds = diamonds %>% 
mutate(lcarat = log(carat), lprice = log(price))
fit = lm(data=diamonds, lprice~lcarat)

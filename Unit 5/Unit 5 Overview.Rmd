...
library(dplyr) #includes stringr
...
left_join(df1,df2, by = "Student_ID") #needs dplyr


#right join 
merge(df1,df2, by = "Student_ID",all.y = TRUE)
right_join(df1,df2, by = "Student_ID") #needs dplyr
...
Acu = read.csv('C:/Users/aleppla/GitRepos/MDS-6306-Doing-Data-Science/Unit 5/Acuspike Customer Data.csv',header = TRUE) # read in company data
...
#WHO removed from CRAN for policy violation, archived
...
#Escape function \ tells R that the following " is part of the string
writeLines(string2)
...
string4
str_sub(string4,1,1)
str_sub(string4,1,7) #String characters 1-7
str_sub(string4,-3,-1) #Last 3 characters
str_sub(string4,-14,-1) #Last 14 characters
str_sub(string4,3,7) #String characters 3-7
```
...
str_view(string5, "shot") #first hit only for "shot"
str_view_all(string5,"shot") #all hits for shot
str_view_all(string5,"shot.") #wildcard, shot.=shot*=shot...
str_view_all(string5,"shot\\.") #use double escape character \\ to escape . (wildcard) and " (string) to find "shot." 
str_view_all(string5,"\\bshot\\b") #Important for text mining, only finds " shot "
str_view_all(string5,"I")
str_view_all(string5," I ")
str_view_all(string5,"\\. I") #Find "I" after a sentence (assumes single space after period) 
str_view_all(string5,"(\\. I|\"I)") #Find "I" after ". " or after a quotation mark(")
#How can we count how many times it comes up in the string?  The answer is coming.

string2 = "\"Don't let what you can't do interfere with what you can do.\" - John Wooden"

str_view(string2, "do.") # . is the wildcard character, but doesn't include "Do" (case sensitive) 
str_view(string2, "do\\.") # need \\. because \ is an escape in both strings and regular expressions

#How to find both "do" and "do."?
str_view_all(string2, "do.")
```

##Anchors
```{r}
str_view(string1, "^Don't") #Does it start with "Don't"?  If so, print string and highlight "Don't"
str_view(string1, "^interfere")
str_view(string3, "^T") #Do any of the string start with "T"?  If so, print string and highlight "T"
...
str_view(string6, "Jobs$") #$ is for anchor at the end

#Find digits and whitespace
str_view(string5,"\\d") #show first digit (\d)
str_view_all(string5,"\\d+")
str_view_all(string6,"\\s") #Ahh... there was white space (\s)
...
df$quotes
str_view(df$quotes,"(Steve|Jobs)")
str_view_all(df$quotes,"[abc]") #finds all letters a, b, or c
str_view_all(df$quotes,"[^abc]") #finds all letters NOT a, b, or c
```
...
str_view(df$quotes, "0+") #Greedy - finds the most sequential 0s for each quote
str_view(df$quotes, "[sn]+") #Greedy - finds the most sequential s's or n's for each quote (case sensitive)
str_view_all(df$quotes, "[sn]+") #finds all s's and n's in all quotes
...
#Allows you to add options to string filters/logic, like ignore_case 
...
NYTIMES_KEY = "oxQiJjy4J4GKupgC5XfaiCg32PFfN7E1" #Your Key Here … get from NTY API website
...
#Same output with geom_col, simpler
allNYTSearch[!is.na(allNYTSearch$NewsOrOther),] %>% 
  group_by(NewsOrOther) %>%
  dplyr::summarize(count=n()) %>%
  mutate(percent = (count / sum(count))*100) %>%
  ggplot() +
  geom_col(aes(y=percent, x=NewsOrOther, fill=NewsOrOther)) + coord_flip()
...
numNewsArticles = dim(newsArticles)[1] #[1] = number of rows
...
for(i in 1 : length(importantWords)) #for each important word in the headline
{
  ...
  numNews = sum(str_count(newsArticles$response.docs.headline.main[-4],importantWords[i]))  #Leave out article to classify = 4
  #number of Other articles that have the ith word in the headline of interest
   numOther = sum(str_count(otherArticles$response.docs.headline.main[-4],importantWords[i]))
